...
class Mascota():
	def __init__(self, codigo, nombre, especie, raza, sexo, edad, dueno):
		self.id = codigo
		self.nombre = nombre
	...
	def agregarVacuna(self, vacuna, fecha):
		...
		especieInluida = False
		for linea in vacunas:
			linea = linea.strip("\n").split(",")
			especie = linea[0]
			if especie == self.especie:
				especieInluida = True; break
		if especieInluida:
			compatible = False
			for linea in vacunas:
				linea = vacunas.strip("\n").split(",")
		...
		else: mostrar("Las vacunas para esta especie aun no estan disponibles")
	def agregarBano(self,fecha):
		open("mascota_bano.txt","a").write(self.id+","+fecha+"\n")
		open("visitas.txt","a").write(self.id+",Bano,"+fecha+"\n")
	...
	def numeroVisitas(self):
		...
		visitas += self.vecesEnArchivo("mascota_bano.txt")
...
class Ingreso(Tk):
	def __init__(self):
		...
		Label(text="User:", fg=fgLbl, font=ftLbl).place(x=70,y=80)
		Label(text="Contraseña:", fg=fgLbl, font=ftLbl).place(x=70,y=120)
		Label(text="Bienvenido, ingresa tu Nombre de Usuario y Password", font=ftTit).place(x=30,y=20)

		self.entNombre = Entry(self, textvariable=self.enter1, highlightcolor="#f00", highlighthickness="1")
		self.entNombre.place(x=170,y=83)
		self.entContra = Entry(self, textvariable=self.enter2, show="*", highlightcolor="#f00", highlighthickness="1")
		...
		ftBtn = ("Calibri",12);
		inicio = Button(self, text="Ingresar", bg="#090", fg="#fff", width="10", command=self.iniciarSes)
		inicio.place(x=90,y=165)
		salir = Button(self, text="Salir", bg="#900", fg="#fff", width="10", command=self.destroy)
	...
	def iniciarSes(self, event=""):
		...
		for linea in users:
			linea = linea.strip("\n").split(",")
			user = self.enter1.get(); passw = self.enter2.get()
			if len(linea)==2 and existe(user,linea[0]) and existe(passw,linea[1]):
				self.withdraw(); Menu()
				malIngreso = False; break
		if malIngreso:
			c+=1
			if(c<3): mostrar("Inicio de sesión incorrecto")
			else: self.destroy()
...
class Menu(Tk):
	def __init__(self):
		...
		Label(self,text="Menú Principal",bg=b,fg=n,font=ftTit).place(x=30,y=15)
		Label(self,text="+"*60,bg=b,fg=n,font=ftLbl).place(x=30,y=56)
		Label(self,text="Añadir Cliente",bg=b,fg=n,font=ftLbl).place(x=60,y=90)
		...
		self.entCodCli = Entry(self,textvariable=self.cli,highlightcolor="#009",highlighthickness="1",width="8",font=ftLbl)
		self.entCodCli.place(x=230,y=125)

		def indexar(): self.destroy(); Indexar()
		def acceder():
			lineas = open("clientes.txt","r").readlines()
			existe = False; lSc = None
			for linea in lineas:
				linea = linea.strip("\n").split(",")
				if linea[0] == self.cli.get():
					...
					existe = True; break;
			if existe:
				cliente = Cliente(lSc[0],lSc[1],lSc[2],lSc[3],lSc[4])
				self.destroy(); VentanaClie(cliente)
			else: mostrar("Código no válido")
		def mostrarCli(): self.destroy(); MostrarCli()
		def mostrarMas(): self.destroy(); MostrarMas()

		btn1 = Button(self,text="Añadir",command=lambda: indexar(), width="12")
...
class Indexar(Tk):
	def _init_(self):
		...
		Label(self,text="Código:",bg=b,fg=n,font=ftLbl).place(x=50,y=90)
		...
		self.nm = Entry(self,textvariable=self.n,highlightcolor="#090",highlighthickness="1"); self.nm.place(x=170,y=125)
		self.ap = Entry(self,textvariable=self.a,highlightcolor="#090",highlighthickness="1"); self.ap.place(x=170,y=160)
		self.sx = Entry(self,textvariable=self.s,highlightcolor="#090",highlighthickness="1"); self.sx.place(x=170,y=195)
		self.ed = Entry(self,textvariable=self.e,highlightcolor="#090",highlighthickness="1"); self.ed.place(x=170,y=230)

		indexar = Button(self,text="Añadir",bg="#090",fg="#fff",width="10",command=self.indexar); indexar.place(x=63,y=280)
		limpiar = Button(self,text="Limpiar campos",bg="#009",fg="#fff",width="15",command=self.limpiar); limpiar.place(x=163,y=280)
		atras = Button(self,text="Atrás",bg="#900",fg="#fff",width="7",command=self.atras); atras.place(x=252,y=22)

	def atras(self): self.destroy(); Menu()
	def limpiar(self): self.n.set("");self.a.set("");self.s.set("");self.e.set("")
	def indexar(self):
		def equal(n,a,s,e):
			return self.n.get()==n and self.a.get()==a and self.s.get()==s and self.e.get()==e
		def write():
			return self.n.get()+","+self.a.get()+","+self.s.get()+","+self.e.get()+"\n"
		if not esNumero(self.e.get() or int(self.e.get())<=a):
			self.limpiar(); mostrar("Ingreso de edad incorrecto")
		elif not self.s.get() in "af":
...
class VentanaCli(Tk):
	def _init_(self):
		...
		Label(self,text=cliente.id,bg=b,fg=n,font=ftTit).place(x=160,y=90)
		...
		Label(self,text=cliente.cantMascotas,bg=b,fg=n,font=ftLbl).place(x=160,y=215)
	...
	self.nomMas = Entry(self,textvariable=self.mas,highlightcolor="#009",highlighthickness="1",width="21",font=ftLbl)
	self.nomMas.place(x=50,y=330)
	access = Button(self, text="Y", command=acceder)
	access.place(x=230,y=330)

	atras = Button(self,text="Atras",bg="#900",fg="#fff",width="7",command="atras"); atras.place(x=205,y=22)
	agreg = Button(self,text="Agregar mascota",width="27",command=agregar); agreg.place(x=49,y=260)

class AgregarMas(Tk):
	def _init_(self,dueno):
		Toplevel._init_(self)
		...
		Label(self,text="Sexo: ",bg=b,fg=n,font=ftLbl).place(x=50,y=230)
		...
		Label(self,text=self.dueno_Id,bg=b,fg=n,font=ftLbl).place(x=170,y=300)

		self.n = StringVar(); self.es = StringVar(); self.r = StringVar(); self.s = StringVar(); self.ed = StringVar()
		self.nm = Entry(self,textvariable=self.n,highlightcolor="#090",highlighthickness="1");self.nm.place(x=170,y=125)
		self.esp = Entry(self,textvariable=self.es,highlightcolor="#090",highlighthickness="1");self.esp.place(x=170,y=160)
		self.ra = Entry(self,textvariable=self.r,highlightcolor="#090",highlighthickness="1");self.ra.place(x=170,y=195)
		self.sx = Entry(self,textvariable=self.s,highlightcolor="#090",highlighthickness="1");self.sx.place(x=170,y=230)
		self.eda = Entry(self,textvariable=self.ed,highlightcolor="#090",highlighthickness="1");self.eda.place(x=170,y=265)

		agregar = Button(self,text="Agregar",bg="#090",fg="#fff",width="10",command=self.agregar); agregar.place(x=63,y=350)
		limpiar = Button(self,text="Limpiar campos",bg="#009",fg="#fff",width="15",command=self.limpiar); limpiar.place(x=163,y=350)
		atras = Button(self,text="< Atras",bg="#900",fg="#fff",width="7",command=self.atras); atras.place(x=252,y=22)

	def atras(self): self.destroy(); VentanaCli(self.dueno)
	def limpiar(self): self.n.set("");self.es.set("");self.r.set("");self.s.set("");self.ed.set("")
	def agregar(self):
		def equal(n,es,r,s,ed):
			return self.n.get()==n and self.es.get()==es and self.r.get()==r and self.s.get()==s and self.ed.get()==ed
		def write():
			return self.n.get()+","+self.es.get()+","+self.r.get()+","+self.s.get()+","+self.ed.get()+","+self.dueno_Id+"\n"
		if not esNumero(self.ed.get()) or int(self.ed.get())<=0:
			...
			self.limpiar()
			mostrar("Agregado correctamente")

class VentanaMas(Tk):
	def _init_(self,dueno,mascota):
		Toplevel._init_(self)
		...
		Label(self,text=mascota.dueno,bg=b,fg=n,font=ftLbl).place(x=160,y=240)
		Label(self,text=mascota.cantVisitas,bg=b,fg=n,font=ftLbl).place(x=160,y=265)

		self.fec = StringVar()
		Label(self,text="Fecha",bg=b,fg=n,font=ftLbl).place(x=50,y=335)
		self.fech = Entry(self,textvariable=self.fec,highlightcolor="#009",highlighthickness="1",font=ftLbl,width="10")
		self.fech.place(x=160,y=335)

		lin1 = "Introduce la fecha en formato (yyyy/mm/dd). Solo realiza UNA"
		lin2 = "operación a la vez (agregar baño, corte, atención y vacuna)."
		...
		Label(self,text="Atención: ",bg=b,fg=n,font=ftLbl).place(x=350,y=240)
		self.vac = Entry(self,textvariable=self.va,highlightcolor="#009",highlighthickness="1",font=ftLbl,width="38")
		self.vac.place(x=435,y=215)
		self.ate = Entry(self,textvariable=self.at,highlightcolor="#009",highlighthickness="1",font=ftLbl,width="38")
		self.ate.place(x=435,y=240)

		def atras(): self.destroy(); VentanaCli(dueno)

		atras = Button(self,text="< Atras",bg="#900",fg="#fff",width="2",command=atras); atras.place(x=702,y=22)

		regVac = Button(self,text="Agregar Vacuna",width="27",command=self.registrarVacuna); regVac.place(x=350,y=300)
		regAte = Button(self,text="Agregar Atención",width="27",command=self.registrarAtencion); regAte.place(x=560,y=300)
		regBan = Button(self,text="Agregar Baño",width="27",command=self.registrarBano); regBan.place(x=350,y=335)
		regCor = Button(self,text="Agregar Corte",width="27",command=self.registrarCorte); regCor.place(x=560,y=335)

	def compFecha(self):
		def esFecha(fecha):
			esAnio = esNumero(fecha[:4]) and int(fecha[:4]) >= 2019
			esMes = esNumero(fecha[5:7]) and int(fecha[5:7]) in range(1,13)
			esDia = esNumero(fecha[8:]) and int(fecha[8:]) in range(1,32)
			return esAnio and esMes and esDia
	...
	def registrarAtencion(self):
		if self.compFecha():
			...
			else: mostrar("Formato de Atención incorrecto")
		else: mostrar("Fecha incorrecta")

	def registrarBano(self):
		if self.compFecha():
			linea = self.mascota.id+","+self.fec.get()+"\n"
			open("mascota_bano.txt","a").write(linea)
...
class MostrarCli(Tk):
	def _init_(self):
		Toplevel._init_(self)
		self.title("Mostrar Cientes")
		self.resizable(0,0)
		self.config(bg="#fff")

		ftLbl = ("Calibri",12); ftTit = ("Calibri",20); n = "#000"; b = "#fff"
		Label(self,text="Información General",bg=b,fg=n,font=ftTit).place(x=30,y=15)
		...
		canvas.create__window((0,0),window=frame,anchor="nw")
		...
		atras = Button(self, text="< Atras", bg="#900", fg="#fff", width="7", command=atras); atras.place(x=393,y=22)
	...
	def _init_(self):
		...
		def show(frame):
			...
			for i in open("mascotas.txt","r"):
				...
				Label(frame,text=k[3]).grid(row=contador,column=3)
				Label(frame,text=k[4]).grid(row=contador,column=4)
				Label(frame,text=k[4]).grid(row=contador,column=5)
				Label(frame,text=k[4]).grid(row=contador,column=6)
...
def mostrar(mensaje):
	...
	bnt.grid(column=1,row=3)
	gen.mainloop()

def nuevoCodig(archivo):
	lineas = open(archivo,"r").readlines()
	ultimo = lineas[-1].strip("\n").split(",") #verificar en codigo estaban dos \
	antCod = int(ultimo[0])
	newCod = str(antCod+1)
